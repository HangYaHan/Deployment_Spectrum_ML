...
def load_model(model_dir: str = "src/models") -> Any:
    """使用 `ModelLoader` 加载模型并返回 loader 实例。
    返回的对象应实现 `predict(features)` 接口。
    """
    print(f"[INIT] ModelLoader(dir='{model_dir}') -> 尝试加载模型")
    try:
        loader = ModelLoader(model_dir=model_dir)
        loader.load()
        print("[INIT] ModelLoader loaded successfully")
        return loader
    except Exception as e:
        print(f"[ERROR] Failed to load model: {e}")
        # 返回占位对象，保证后续流程不会因 None 直接崩溃
        return {"type": "Model", "path": model_dir, "loaded": False}
